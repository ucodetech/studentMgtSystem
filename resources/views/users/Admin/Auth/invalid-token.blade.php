...
<div class="card">
    <div class="card-body">
        <div class="m-sm-3">
           ...
           <form action="{{ route('admin.ad.resend.token') }}" method="post">
            @csrf
            @method("POST")
            <div class="mb-3">
              <label for="" class="form-label">Emaill</label>
              <input type="email" name="admin_email" id="admin_email" class="form-control" placeholder="Entered your registered email address" value="{{ session()->has('registered_admin_email')? session()->get('registered_admin_email') : "" }}" aria-describedby="helpId">
